export const NOTE_TYPE_ID = 1761782400004;
export const NOTE_TYPE_NAME = "yt2anki";
export const PINYIN_NOTE_TYPE_ID = 1761782400005;
export const PINYIN_NOTE_TYPE_NAME = "yt2anki-pinyin";
...
export async function createSegmentIdentity(input: {
  ...
  return `v4_${base64Url(await sha256(canonical))}`;
}

export function noteGuid(identity: string): string {
  const digest = identity.slice(3);
  return `y4a_${digest.slice(0, 22)}`;
}

export function pinyinNoteGuid(identity: string): string {
  const digest = identity.slice(3);
  return `y4p_${digest.slice(0, 22)}`;
}
